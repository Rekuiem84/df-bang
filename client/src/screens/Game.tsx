...
export function Game({ api }: { api: GameApi }) {
	...
	const [hints, setHints] = useState(
		() => localStorage.getItem("bang_hints") !== "off",
	);
	...
	useEffect(() => {
		...
		const name =
			g.players.find((p) => p.id === g.currentPlayerId)?.pseudo ?? "…";
	...
	const GUESS_CYCLE: (Role | undefined)[] = [
		undefined,
		"outlaw",
		"renegade",
		"deputy",
	...
	function isValidTarget(card: Card, p: PublicPlayer): boolean {
		...
		switch (card.name) {
			...
			case "missed": // Calamity Janet : joue un Raté! comme un BANG!
				return me!.character === "calamity_janet" && dist <= myWeaponRange;
			case "panic":
				return dist <= 1;
			case "duel":
			case "cat_balou":
	...
	/** Carte nécessitant une cible (Raté! seulement pour Calamity Janet). */
	function needsTarget(card: Card): boolean {
		if (NEEDS_TARGET.has(card.name)) return true;
		return card.name === "missed" && me!.character === "calamity_janet";
	}

	const aliveCount = game.players.filter((p) => p.isAlive).length + 1; // + moi
	/** Vrai si la carte est réellement jouable maintenant (sinon grisée/bloquée). */
	function cardUsable(card: Card): boolean {
		if (!canPlay) return false;
		const hasTarget = (pred: (p: PublicPlayer) => boolean) =>
			game!.players.some(pred);
		switch (card.name) {
			case "missed":
				// Calamity Janet peut jouer un Raté! comme un BANG!.
				return (
					me!.character === "calamity_janet" &&
					game!.canPlayBang &&
					hasTarget((p) => isValidTarget(card, p))
				);
			case "beer":
				return me!.hp < me!.maxHp && aliveCount > 2;
			case "bang":
				return game!.canPlayBang && hasTarget((p) => isValidTarget(card, p));
			case "duel":
			case "jail":
				return hasTarget((p) => isValidTarget(card, p));
			case "panic":
			case "cat_balou":
				return hasTarget(
					(p) =>
						isValidTarget(card, p) && (p.handCount > 0 || p.inPlay.length > 0),
				);
			case "barrel":
			case "mustang":
			case "scope":
				return !me!.inPlay.some((c) => c.name === card.name);
			case "dynamite":
				return !me!.inPlay.some((c) => c.name === "dynamite");
			default:
				return true; // saloon, pioches, magasin, indiens, gatling, armes
		}
	}
	/** Carte bloquée à cause du mode indices (grisée + non glissable). */
	function isBlocked(card: Card): boolean {
		return hints && canPlay && !cardUsable(card);
	}
	...
	function onPointerMove(e: React.PointerEvent) {
		...
		if (g.moved && canPlay && !isBlocked(g.card)) {
	...
	function onPointerUp(card: Card) {
		...
		setDrag(null);
		if (moved) {
			if (armed && canPlay && !isBlocked(card)) playOrTarget(card);
		} else {
			// Clic simple sans déplacement → aperçu agrandi (même si grisée).
			setPreview(card);
		}
	}
	...
	return (
		<div className="screen game">
			...
			{/* Sous-barre : historique + toggle indices visuels */}
			<div className="subbar">
				<button className="btn tiny ghost" onClick={() => setShowLog(true)}>
					📖 Historique
				</button>
				<button
					className={`btn tiny ${hints ? "primary" : "ghost"}`}
					title="Griser les cartes non jouables ce tour"
					onClick={() =>
						setHints((h) => {
							localStorage.setItem("bang_hints", h ? "off" : "on");
							return !h;
						})
					}>
					💡 Aide
			...
			<div className="table">
				{game.players.map((p, i) => (
					<div
						key={p.id}
						className="seat-slot"
						style={seatPosition(i, game.players.length)}>
						<TableSeat
							player={p}
				...
				<DiscardPile
					cards={game.discardRecent}
					armed={!!drag?.armed}
					onPreview={setPreview}
					theme={theme}
				/>
				<div className="deck-count">🂠 {game.deckCount}</div>
			...
			<div className={`me-zone ${isMyTurn ? "active" : ""}`}>
				<div className="me-status">
					...
					<span className={`role role-${me.role}`}>
						{roleLabel(me.role, theme)}
					</span>
					<span
						className="me-char info"
						onClick={() => setInfoChar(me.character)}>
				...
				<div className="fan">
					{me.hand.map((c, i) => {
						...
						return (
							<div
								key={c.id}
								className={`fan-card ${canPlay ? "playable" : ""} ${isBlocked(c) ? "unplayable" : ""} ${isDragged && drag!.armed ? "armed" : ""}`}
								style={style}
				...
				<div className="turn-controls">
					{me.character === "sid_ketchum" &&
						me.hp < me.maxHp &&
						me.hand.length >= 2 && (
							<SidKetchum api={api} hand={me.hand} theme={theme} />
						)}
					{canPlay && (
						<button className="btn primary end-turn" onClick={api.endTurn}>
							Finir le tour
						</button>
					)}
				</div>
			...
			{preview && (
				<div className="card-preview-overlay" onClick={() => setPreview(null)}>
					<div className="card-preview">
						<CardView card={preview} theme={theme} image />
					</div>
				</div>
			)}
...
function fanTransform(
	...
	const rot = t * 12; // degrés (extrémités à ±6° — éventail peu incliné)
	const dip = Math.abs(t) * 9; // les cartes extérieures descendent
	return {
